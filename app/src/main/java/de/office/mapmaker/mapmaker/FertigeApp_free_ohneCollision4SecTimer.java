...
public class FertigeApp_free_ohneCollision4SecTimer extends AppCompatActivity {
    ...
    public void drawSpherosWay(int xNew, int yNew) {
        ...
        canvas.drawPoint(canvasCenterX + (xNew * (-2)), canvasCenterY + (yNew * (2)), paint);
    ...
    public void counterForPos() {
        if(positionYList.size() > 26 ) {
            double a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z;

            //will probably work that good
           // int checkedValues = 0;
           // for(int counter = 1; counter <= 25; counter++) {
           //     if(positionYList.get(positionYList.size()-(0)) ==
           //             (positionYList.get(positionYList.size()-(counter)))) {
           //         checkedValues++;
           //         if(checkedValues == 20) {
           //             posTrue = true;
           //             break;
         ...
         // bad solution, but it works for what it should
            a = positionYList.get(positionYList.size()-1);
            b = positionYList.get(positionYList.size()-2);
            c = positionYList.get(positionYList.size()-3);
            d = positionYList.get(positionYList.size()-4);
            e = positionYList.get(positionYList.size()-5);
            f = positionYList.get(positionYList.size()-6);
            g = positionYList.get(positionYList.size()-7);
            h = positionYList.get(positionYList.size()-8);
            i = positionYList.get(positionYList.size()-9);
            j = positionYList.get(positionYList.size()-10);
            k = positionYList.get(positionYList.size()-11);
            l = positionYList.get(positionYList.size()-12);
            m = positionYList.get(positionYList.size()-13);
            n = positionYList.get(positionYList.size()-14);
            o = positionYList.get(positionYList.size()-15);
            p = positionYList.get(positionYList.size()-16);
            q = positionYList.get(positionYList.size()-17);
            r = positionYList.get(positionYList.size()-18);
            s = positionYList.get(positionYList.size()-19);
            t = positionYList.get(positionYList.size()-20);
            u = positionYList.get(positionYList.size()-21);
            v = positionYList.get(positionYList.size()-22);
            w = positionYList.get(positionYList.size()-23);
            x = positionYList.get(positionYList.size()-24);
            y = positionYList.get(positionYList.size()-25);
            z = positionYList.get(positionYList.size()-26);
        ...
        if (mRobot.isConnected()) {
            ...
            final TimerTask myTask = new TimerTask() {
                ...
                public void run() {
                    ...
                    mRobot.sendCommand(new RollCommand(mRobot.getLastHeading(), 0.08f, RollCommand.State.GO));
                    counterForPos();
                }
            };
            timer.schedule(myTask, 0, 2800);
                ...
                public void handleAsyncMessage(AsyncMessage asyncMessage, Robot robot) {
                    if (asyncMessage instanceof DeviceSensorAsyncMessage) {
                        ...
                        if (posTrue) {
                            ...
                            mRobot.drive(mRobot.getLastHeading() + 127, 0);
                            posTrue = false;
                        }
